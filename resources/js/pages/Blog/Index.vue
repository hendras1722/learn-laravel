  ...
  <AppLayout>
    <template #header>
      <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight"><a
        ...
        <PrimaryLink :href="route('blogs.index', { filter: 'only' })" class="max-w-xl ml-2">View Trashed</PrimaryLink>
      </h2>
    </template>
    <div class="py-12">
      <div>
        <PrimaryLink :href="route('dashboard', { filter: 'only' })" class="max-w-xl ml-2">View Trashed</PrimaryLink>
      </div>
          ...
          <table class="w-full whitespace-nowrap">
            ...
            <tbody>
              <tr v-for="entry in props.blogs.data" :key="entry.id" class="hover:bg-gray-100 focus-within:bg-gray-100">
                ...
                <td class="border-t">
                  <PrimaryLink v-if="entry.deleted_at == null" :href="route('blogs.show', { 'id': entry.id })"
                    class="max-w-xl ml-2">VIEW</PrimaryLink>
                  <PrimaryLink v-if="entry.deleted_at == null" :href="route('blogs.edit', { 'id': entry.id })"
                    class="max-w-xl ml-2">EDIT</PrimaryLink>
